...
func (p *PortalProtocol) portMappingLoop() {
	...
	defer func() {
		...
		for _, m := range mappings {
			if m.extPort != 0 {
				logger := newLogger(m.protocol, m.extPort, m.port)
				logger.Debug("Deleting port mapping")
				err := p.NAT.DeleteMapping(m.protocol, m.extPort, m.port)
				if err != nil {
					logger.Error("Couldn't delete port mapping", "err", err)
					return
				}
			}
		}
	...
	for {
		...
		case <-refresh.C():
			for _, m := range mappings {
				...
				logger := newLogger(m.protocol, external, m.port)

				logger.Trace("Attempting port mapping")
				...
				if external != m.extPort {
					logger = newLogger(m.protocol, m.extPort, m.port)
					logger.Info("NAT mapped alternative port")
				} else {
					logger.Info("NAT mapped port")
				}
