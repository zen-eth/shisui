...
// Hash: 18c9e7650b34da01c10098956fec4691d2256ac3fa6674d879fbb63de7926eb7
// Version: 0.1.3
package portalwire
...
func (p *Ping) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	// Field (1) 'PayloadType'
	dst = ssz.MarshalUint16(dst, p.PayloadType)

	// Offset (2) 'Payload'
	dst = ssz.WriteOffset(dst, offset)

	// Field (2) 'Payload'
	if size := len(p.Payload); size > 1100 {
		err = ssz.ErrBytesLengthFn("Ping.Payload", size, 1100)
		return
	}
...
func (p *Ping) UnmarshalSSZ(buf []byte) error {
	...
	// Field (1) 'PayloadType'
	p.PayloadType = ssz.UnmarshallUint16(buf[8:10])

	// Offset (2) 'Payload'
	if o2 = ssz.ReadOffset(buf[10:14]); o2 > size {
...
func (p *Ping) HashTreeRootWith(hh ssz.HashWalker) (err error) {
	...
	// Field (1) 'PayloadType'
	hh.PutUint16(p.PayloadType)

	// Field (2) 'Payload'
	{
		elemIndx := hh.Index()
		byteLen := uint64(len(p.Payload))
		if byteLen > 1100 {
			err = ssz.ErrIncorrectListSize
			return
		}
		hh.Append(p.Payload)
		hh.MerkleizeWithMixin(elemIndx, byteLen, (1100+31)/32)
	}
...
func (p *Pong) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	// Field (2) 'Payload'
	if size := len(p.Payload); size > 1100 {
		err = ssz.ErrBytesLengthFn("Pong.Payload", size, 1100)
		return
	}
	dst = append(dst, p.Payload...)
...
// MarshalSSZ ssz marshals the AcceptV1 object
func (a *AcceptV1) MarshalSSZ() ([]byte, error) {
	return ssz.MarshalSSZ(a)
}

// MarshalSSZTo ssz marshals the AcceptV1 object to a target array
func (a *AcceptV1) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	dst = buf
	offset := int(6)

	// Field (0) 'ConnectionId'
	if size := len(a.ConnectionId); size != 2 {
		err = ssz.ErrBytesLengthFn("AcceptV1.ConnectionId", size, 2)
		return
	}
	dst = append(dst, a.ConnectionId...)
	...
	// Field (1) 'ContentKeys'
	if size := len(a.ContentKeys); size > 64 {
		err = ssz.ErrListTooBigFn("AcceptV1.ContentKeys", size, 64)
		return
	}
	for ii := 0; ii < len(a.ContentKeys); ii++ {
		dst = ssz.MarshalUint8(dst, a.ContentKeys[ii])
	}

	return
}

// UnmarshalSSZ ssz unmarshals the AcceptV1 object
func (a *AcceptV1) UnmarshalSSZ(buf []byte) error {
	var err error
	size := uint64(len(buf))
	if size < 6 {
		return ssz.ErrSize
	...
	{
		buf = tail[o1:]
		num, err := ssz.DivideInt2(len(buf), 1, 64)
		if err != nil {
			return err
		}
		a.ContentKeys = ssz.ExtendUint8(a.ContentKeys, num)
		for ii := 0; ii < num; ii++ {
			a.ContentKeys[ii] = ssz.UnmarshallUint8(buf[ii*1 : (ii+1)*1])
		}
	}
	return err
}

// SizeSSZ returns the ssz encoded size in bytes for the AcceptV1 object
func (a *AcceptV1) SizeSSZ() (size int) {
	size = 6

	// Field (1) 'ContentKeys'
	size += len(a.ContentKeys) * 1

	return
}

// HashTreeRoot ssz hashes the AcceptV1 object
func (a *AcceptV1) HashTreeRoot() ([32]byte, error) {
	return ssz.HashWithDefaultHasher(a)
}

// HashTreeRootWith ssz hashes the AcceptV1 object with a hasher
func (a *AcceptV1) HashTreeRootWith(hh ssz.HashWalker) (err error) {
	indx := hh.Index()
	...
	// Field (1) 'ContentKeys'
	{
		...
		subIndx := hh.Index()
		for _, i := range a.ContentKeys {
			hh.AppendUint8(i)
		}
		hh.FillUpTo32()
		numItems := uint64(len(a.ContentKeys))
		hh.MerkleizeWithMixin(subIndx, numItems, ssz.CalculateLimit(64, numItems, 1))
	}
...
// GetTree ssz hashes the AcceptV1 object
func (a *AcceptV1) GetTree() (*ssz.Node, error) {
	return ssz.ProofTree(a)
}
