...
// See original source: https://github.com/optimism-java/shisui/blob/06751b3ec9fec0b4bbead878bbb7a502dc728692/internal/flags/helpers.go
//
		...
		for _, parent := range ctx.Lineage()[1:] {
			if parent.IsSet(name) {
				...
				if result := parent.StringSlice(name); len(result) > 0 {
					...
					if err != nil {
						log.Error("Failed to set flag", "name", name, "err", err)
						return
					}
				} else {
